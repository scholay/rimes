...
try {
    ...
    Assert-PreviewTest -Condition ([int]$buildResult.PayloadFiles -eq 55) -Message 'reviewed payload is not the expected 55-file closure'
    ...
    Assert-PreviewTest -Condition ($payloadFiles.Count -eq 55) -Message 'extracted ZIP does not contain exactly 55 payload files'
