...
if (-not $SkipPlatformCheck -and -not $runningOnWindows) {
    throw 'TSF unregistration is supported only on Windows.'
}
...
$plan = [pscustomobject]@{
    ...
    ChangesDefaultKeyboard = $false
}

if ($DryRun) {
    if ($runningOnWindows -and -not $SkipPlatformCheck) {
        Invoke-RimesRegistrar -Executable $registrar -Arguments @('unregister', '--dll', $dllPath, '--dry-run') | Out-Host
    }
    $plan
    return
}
if (-not (Test-RimesElevated)) {
    throw 'Unregister-RimesWindows.ps1 must run from an elevated PowerShell session.'
}

Invoke-RimesRegistrar -Executable $registrar -Arguments @('unregister', '--dll', $dllPath) | Out-Host
Invoke-RimesRegistrar -Executable $registrar -Arguments @('verify-absent', '--dll', $dllPath) | Out-Host

$plan | Add-Member -NotePropertyName VerifiedAbsent -NotePropertyValue $true -PassThru
