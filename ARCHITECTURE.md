...
> **2026-07-22 意识流输入基线（并击与 Space 语义以 2026-07-26 覆盖为准）**：内置 `builtin.stream-input` 只在缓冲持久开启、插件为唯一 owner、secure input 关闭且外部 `FocusToken` 精确存活时于 Rime 前捕获。raw 拼音与目标块只存在 `StreamInputWorkspace`，不写入 Rime/CompositionSession/BufferModel。普通 raw 变更只重置 220 ms debounce，800 ms burst 上限不重置；最多允许旧视觉 producer 与新 challenger 两路短暂重叠，跨 revision 旧结果、partial 与 baseline 都不可进入新 prompt 或获得投递权，墓碑后的迟到回调作废。workspace 专用 `~/Library/RimeBuffer/ai/openai-compatible.json` 的 OpenAI 兼容配置，并对请求关闭 thinking、要求 JSON object、限制 1024 output tokens。每轮用完整 raw 和最多三条本地 lossless 提示全局重算；合法 final 不足本地下限时只做一次有界排重重试，旧合法结果优先合并、精确去重且最多三条，partial 永不可投递。候选各占独立 target row，行内由宿主安全细分为 block；↑/↓ 或数字切换，Return/纸飞机在首个实际投递前确认所选项并淘汰其余项。raw 贯穿部分投递保留，只在最后一块成功后清除；首块后继续输入则主动丢弃未发尾部，已发送前缀不得复活。
