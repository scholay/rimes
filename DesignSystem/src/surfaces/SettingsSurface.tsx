...
const MAILBOX_THREADS: readonly MailboxThread[] = [
  {
    id: "thread-today-03",
    dateLabel: "今天",
    sequence: "#03",
...
export function SettingsSurface({
  ...
  const [selectedMailboxThreadID, setSelectedMailboxThreadID] = useState(
  ...
  const renderPage = () => {
    ...
    if (currentRoute.id === "core.buffer") {
      ...
      if (currentSubpage === "mailbox") {
        const selectedThread = MAILBOX_THREADS.find(
          (thread) => thread.id === selectedMailboxThreadID,
        ) ?? MAILBOX_THREADS[0];
        const mailboxGroups = MAILBOX_THREADS.reduce<
        ...
        return (
          <SettingsSection
            className="settings-section--mailbox"
            title="Mailbox"
            description="外部推送以会话形式留在窗口内：左侧按日期与序号浏览，右侧查看完整对话。"
          >
            <div className="mailbox-pane" aria-label="Mailbox 双栏预览">
                  ...
                  <div className="mailbox-index-group" key={group.dateLabel}>
                    ...
                    {group.threads.map((thread) => (
                      <button
                        ...
                        key={thread.id}
                        onClick={() => {
                          setSelectedMailboxThreadID(thread.id);
                          setStatus(`已打开 ${thread.dateLabel} ${thread.sequence}`);
                        }}
                        type="button"
                      >
                  ...
                  <>
                    ...
                    <footer className="mailbox-chat__composer" aria-label="回复草稿">
                      ...
                      <Button
                        icon="send"
                        kind="secondary"
                        onClick={() => setStatus("设计预览：回复不会真正发送")}
                      >
                        发送
                      </Button>
                    </footer>
