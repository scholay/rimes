...
- 分类改名为「临时」（原最近）、「图库」（原图片）、「影集」（原视频），只改变显示名称，不迁移或改写既有数据。
- 底栏所有卡片（含捕获）仅在选中时显示右上角 `⋯`，普通卡片悬停时在右下角显示复制按钮；密码卡片未验证时不显示复制，验证后才提供独立复制按钮。菜单中的编辑、按类型迁移、删除均有图标。无合法目标的迁移项明确禁用，不把图片、PDF 或密码强行转换为文字。悬停复制针对当前悬停卡片，不改变选择，也不自动上屏。
- 「临时」的文字/链接迁移为笔记，图片进入图库，PDF 文件进入 PDF，视频文件进入影集；多个文件逐个校验，全部保存成功（或已经存在）后才移除原历史卡片，部分失败保留来源。普通 `⌘S` 仍是收藏，不删除历史。
- 笔记可以显式迁移为 Password：先加密写入，再按原 revision 移除笔记；并发修改时拒绝覆盖并回滚新条目。操作前说明标题仍可检索，既有同步/备份中的明文副本无法收回。Password 不提供降级成明文笔记的迁移。
- 底栏 Password 卡片双击或回车即可原地验证：四个脱敏进度槽位加一个输入区，不显示说明文字或原始按键。现有四组物理并击口令不变；按住键时显示输入中，每组全部松开后保留槽位进度，输入区恢复等待状态。四组完整匹配后显示成功图标和正文；失败清空槽位并保留错误重试图标，下一次输入直接开始新尝试，不必关掉再打开。锁定及验证期间均无复制按钮，验证不自动复制、不投递。详情使用相同验证界面。明文最多停留 15 秒，切卡片/分类、滚动、搜索、失焦、关闭、保护态、条目或口令变化立即遮蔽。
- 密码明文显示后，右下角常显独立复制按钮；详情中也有显式复制。只有有效查看期且窗口持有焦点、无安全输入保护时才能复制，复制不延长查看期。剪贴板正文带 `org.nspasteboard.ConcealedType` 和 `org.nspasteboard.TransientType`，本应用的临时历史和归档器拒绝收录；不会自动发送或模拟粘贴。查看口令本身始终不进入剪贴板。
...
- `capsule-interaction-smoke` 使用隔离存储和具名临时剪贴板，覆盖四槽脱敏进度、逐组松键回到等待态、错误口令零解密与重试、验证前无复制按钮、显式复制及敏感标记、失效卡片拒绝复制、选中/悬停操作、保护/口令变更清理、创建与紧凑布局、带图标菜单以及迁移的加密和并发边界；不读取真实密码，不碰系统通用剪贴板。
