...
```
 ...
 旧 Marine prepare prompt ─▶ ActionPluginHost ─┐                            │
                                            └─▶ AITextConnectorRegistry    │
                                                ...
                                                ├─ Claude Code（CLI 授权态）   │
                                                └─ OpenAI 兼容 API           │
 意识流 raw ───────────▶ StreamInputWorkspace ─▶ 本地 Rime + Octagram ─┐             │
...
┌──────────────────────┐       ┌─────────────────────────────────────┐
│ CandidateWindow panel │       │ 可切换 Toolbar 33pt + divider 1pt      │
│ 跟随宿主 caret         │       ├─────────────────────────────────────┤
└──────────────────────┘       │ 内容轨：折叠 44/78，展开 78/112pt      │
...
| 网络出站清单 | 更新检查、用户显式调用的 Codex/Claude/OpenAI，以及本地意识流不支持时按现有配置启用的 AI 回退已存在；SSE 订阅/SSH 仍属后续 | ✅ 本地可解码输入不出站；回退状态可见 |
